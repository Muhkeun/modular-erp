...
```
구매요청 --> 결재 --> 구매주문 --> 입고 --> 재고 업데이트 --> 분개 전기
  |                     |           |
  +-- 예산 잔액 확인     +-- 협력사 포탈  +-- 품질 검사
```

### 주문-수금 (O2C)

```
CRM 리드 --> 기회 --> 판매주문 --> 출고 --> 재고 차감 --> 세금계산서 --> 분개 전기
                                                               |
                                                  다중통화 환산 (외화 거래 시)
```
...
```
분개 전기 --> 감가상각 실행 --> 원가 배분 --> 환율 재평가
  |                                           |
  +--> 마감 체크리스트 --> 소프트 마감 --> 하드 마감 --> 재무제표
```
...
```
191개 백엔드 테스트 | 15개 프론트엔드 단위 테스트 | 46개 E2E 시나리오
───────────────────────────────────────────────────────────────────
전체 통과 | 0 실패
```
...
```
main Push --> GitHub Actions --> Docker 빌드 --> AWS ECR --> EC2 배포
```
