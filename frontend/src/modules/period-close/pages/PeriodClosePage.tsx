...
import { periodCloseApi } from "../../../shared/api/periodCloseApi";
...
const statusStyle: Record<string, string> = {
  OPEN: "badge-success",
  SOFT_CLOSE: "badge-warning",
  HARD_CLOSE: "badge bg-slate-100 text-slate-500",
...
export default function PeriodClosePage() {
  ...
  const [form, setForm] = useState({ companyCode: "", fiscalYear: new Date().getFullYear() });

  const { data, isLoading } = useQuery({
    queryKey: ["fiscal-periods"],
    queryFn: async () => (await periodCloseApi.getPeriods()).data,
  });

  const checklistQuery = useQuery({
    queryKey: ["period-checklist", selected?.id],
    queryFn: async () => (await periodCloseApi.getChecklist(selected!.id)).data,
    enabled: mode === "detail" && !!selected?.id && activeTab === "checklist",
  });

  const entriesQuery = useQuery({
    queryKey: ["closing-entries", selected?.id],
    queryFn: async () => (await periodCloseApi.getClosingEntries(selected!.id)).data,
    enabled: mode === "detail" && !!selected?.id && activeTab === "entries",
  });

  const createMut = useMutation({
    mutationFn: async (body: Record<string, unknown>) => (await periodCloseApi.createPeriod(body)).data,
  ...
  const executeTaskMut = useMutation({
    mutationFn: async ({ periodId, taskId }: { periodId: number; taskId: number }) =>
      (await periodCloseApi.executeTask(periodId, taskId)).data,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["period-checklist", selected?.id] }); },
  });

  const softCloseMut = useMutation({
    mutationFn: async (id: number) => (await periodCloseApi.softClose(id)).data,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["fiscal-periods"] }); },
  });

  const hardCloseMut = useMutation({
    mutationFn: async (id: number) => (await periodCloseApi.hardClose(id)).data,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["fiscal-periods"] }); },
  });

  const columnDefs = useMemo<ColDef<FiscalPeriod>[]>(() => [
    { field: "periodName", headerName: t("periodClose.name"), flex: 1.5,
    ...
    { field: "period", headerName: t("periodClose.periodNo"), flex: 0.6 },
  ...
  const entryColDefs = useMemo<ColDef<ClosingEntry>[]>(() => [
    ...
    { field: "entryType", headerName: t("periodClose.entryType", "Entry Type"), flex: 1 },
    { field: "debitAccount", headerName: t("je.debit"), flex: 1 },
    { field: "creditAccount", headerName: t("je.credit"), flex: 1 },
    { field: "amount", headerName: t("costing.amount", "Amount"), flex: 1, type: "numericColumn", valueFormatter: (p: { value: number }) => fmt(p.value) },
    ...
    return (
      <div>
        <PageHeader title={t("periodClose.title")} description={t("periodClose.description")}
          ...
          actions={<button className="btn-primary" onClick={() => { setForm({ companyCode: "", fiscalYear: new Date().getFullYear() }); setMode("create"); }}>
    ...
    return (
      <div>
        ...
        <div className="section-card">
          ...
          <h3 className="section-title">{t("periodClose.generateFiscalYear", "Generate Fiscal Year")}</h3>
  ...
  return (
    <div>
      <PageHeader title={selected?.periodName || ""}
      ...
      <div className="section-card">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="stat-tile">
            <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">{t("nav.companies")}</span>
            <span className="font-medium text-slate-900">{selected?.companyCode || "-"}</span>
          </div>
          <div className="stat-tile">
              ...
              checklist.map(task => (
                <div key={task.id} className="flex items-center justify-between rounded-[22px] border border-slate-200/80 bg-slate-50/70 p-4">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-medium text-slate-400">#{task.sequence}</span>
                    <div>
