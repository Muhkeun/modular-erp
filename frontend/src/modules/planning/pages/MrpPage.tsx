...
export default function MrpPage() {
  ...
  return (
    <div>
      <PageHeader title={t("mrp.title")} description={t("mrp.description")}
      ...
      <div className="card p-6 mb-6">
        ...
        <div className="flex items-end gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">{t("mrp.plantCode")}</label>
            <input className="input w-32" value={plantCode} onChange={e => setPlantCode(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">{t("mrp.horizon")}</label>
        ...
        <div className="card">
          <div className="border-b border-slate-100 px-6 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">{t("gr.plant")}: {latestRun.plantCode} | {latestRun.executedAt || "—"}</p>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    <th className="px-6 py-3 text-left font-semibold text-slate-600">{t("item.name")}</th>
                    <th className="px-4 py-3 text-right font-semibold text-slate-600">{t("mrp.grossReq")}</th>
                    <th className="px-4 py-3 text-right font-semibold text-slate-600">{t("mrp.onHand")}</th>
                    <th className="px-4 py-3 text-right font-semibold text-slate-600">{t("mrp.netReq")}</th>
                    <th className="px-4 py-3 text-right font-semibold text-slate-600">{t("mrp.plannedQty")}</th>
                    <th className="px-4 py-3 text-center font-semibold text-slate-600">{t("mrp.action")}</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-600">{t("mrp.requiredBy")}</th>
                  </tr>
                </thead>
                    ...
                    <tr key={r.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center gap-1.5">
                          ...
                          <span className="text-xs">{String(t("mrp.actionTypes." + r.actionType, r.actionType))}</span>
                        </span>
                      </td>
