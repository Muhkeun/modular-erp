...
export const periodCloseApi = {
  ...
  createPeriod: (data: Record<string, unknown>) => api.post(`${BASE}/periods/generate`, data),
  getChecklist: (id: number) => api.get(`${BASE}/periods/${id}/checklist`),
  executeTask: (periodId: number, taskId: number) => api.post(`${BASE}/periods/${periodId}/tasks/${taskId}/execute`),
